...
```{r eval = FALSE}
maxcluster <- 10
ckm.sel.all <- CKMSelAll(dataset, maxcluster) #the defaults are used for other input arguments
```
...
estimates.no <- summary(ckm.sel.no, dataset, ncluster)
estimates.var <- summary(ckm.sel.var, dataset, ncluster)
estimates.all <- summary(ckm.sel.all, dataset, ncluster)
## example outputs
# estimated number of masking variables
est.nnoisevar <- estimates.no[[1]]
# estimated set of signaling variables
est.signalingset <- estimates.no[[2]]
# estimated cluster assignment
est.cluster <- estimates.no[[3]]
# estimated cluster centroids
est.centroids <- estimates.no[[4]]
```
...
plot(ckm.sel.no, dataset, ncluster)
plot(ckm.sel.var, dataset, ncluster)
plot(ckm.sel.all, dataset, ncluster)
```
