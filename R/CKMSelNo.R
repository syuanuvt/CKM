...
CKMSelNo <- function(dataset, n.cluster, n.noisevar, num_starts_kmeans = 10){
  ckm <- CardKMeans(dataset, n.cluster, n.noisevar, num_starts_kmeans)
  opt.variables <- ckm$variables
  opt.cluster.assign <- ckm$cluster.assign
  results <- list(n.noisevar = n.noisevar, signaling.set = opt.variables, cluster.assign = opt.cluster.assign, opt.cluster = n.cluster, org.data = dataset)
  class(results) <- "ckm"
  return(results)
}
