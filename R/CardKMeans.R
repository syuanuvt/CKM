...
CardKMeans <- function(dataset, n.cluster, n.noisevar, num_starts_kmeans = 10, recal = TRUE){
  ...
  kmeans.results <- kmeans(selection.score, centers = n.cluster, nstart = num_starts_kmeans, iter.max = 50)$cluster
  ...
  kmeans.final <- kmeans(new.data, centers = n.cluster, iter.max = 50)
